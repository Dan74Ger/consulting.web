...
namespace ConsultingGroup.Controllers
{
    ...
    public class Attivita740Controller : Controller
    {
        private readonly ApplicationDbContext _context;

        public Attivita740Controller(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index(int? annoSelezionato)
        {
            ...
            // Carica TUTTE le attività 740 esistenti per l'anno selezionato
            var attivitaEsistenti = await _context.Attivita740
                .Include(a => a.Cliente)
            ...
            foreach (var cliente in clientiSenzaAttivita)
            {
                ...
                _context.Attivita740.Add(nuovaAttivita);
            }
            ...
            if (clientiSenzaAttivita.Any())
            {
                ...
                attivitaEsistenti = await _context.Attivita740
                    .Include(a => a.Cliente)
                    .Include(a => a.Professionista)
                    .Include(a => a.AnnoFiscale)
                    .Where(a => a.IdAnno == annoFiscale.IdAnno)
        ...
        // POST: Attivita740/BulkUpdate
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> BulkUpdate(List<Attivita740> attivita, int? annoSelezionato, IFormCollection form)
        {
            ...
            foreach (var item in attivita.Where(a => a.IdAttivita740 > 0))
            {
                try
                {
                    var esistente = await _context.Attivita740.FindAsync(item.IdAttivita740);
                    if (esistente != null)
                    {
        ...
        public async Task<IActionResult> DeleteSingle(int id)
        {
            try
            {
                ...
                var attivita = await _context.Attivita740
                    .Include(a => a.Cliente)
                    .Include(a => a.AnnoFiscale)
                    .FirstOrDefaultAsync(a => a.IdAttivita740 == id);
                ...
                // Elimina l'attività
                _context.Attivita740.Remove(attivita);
                await _context.SaveChangesAsync();

                // Log dell'operazione
                Console.WriteLine($"Eliminata attività Mod. 740 per cliente: {nomeCliente} (Anno: {anno})");

                return Json(new { 
                    success = true, 
                    message = $"Attività Mod. 740 per il cliente \"{nomeCliente}\" eliminata con successo."
                });
            }
            catch (Exception ex)
            {
        ...
        // POST: Attivita740/DeleteAllForYear
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteAllForYear(int anno)
        {
            try
            {
                ...
                var attivitaDaEliminare = await _context.Attivita740
                    .Where(a => a.IdAnno == annoFiscale.IdAnno)
                    .ToListAsync();
                ...
                _context.Attivita740.RemoveRange(attivitaDaEliminare);
                await _context.SaveChangesAsync();

                // Log dell'operazione (opzionale)
                Console.WriteLine($"Eliminati {count} inserimenti Mod. 740 per l'anno {anno}");
        ...
        public async Task<IActionResult> ExportExcel(int anno)
        {
            try
            {
                ...
                // Ottieni tutti i dati delle attività 740 per l'anno
                var attivita = await _context.Attivita740
                    .Include(a => a.Cliente)
                    .Include(a => a.Professionista)
                    .Include(a => a.AnnoFiscale)
                ...
                var worksheet = package.Workbook.Worksheets.Add($"Mod 740 - Anno {anno}");
                ...
                worksheet.Tables.Add(dataRange, "Mod740Data");
                var table = worksheet.Tables["Mod740Data"];
                table.ShowHeader = true;
                table.ShowFilter = true;
                table.TableStyle = OfficeOpenXml.Table.TableStyles.Medium2;

                // Crea il file
                var fileName = $"Mod740_Anno_{anno}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
