...
namespace ConsultingGroup.Controllers
{
    ...
    public class Attivita760Controller : Controller
    {
        private readonly ApplicationDbContext _context;

        public Attivita760Controller(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index(int? annoSelezionato)
        {
            ...
            // Carica TUTTE le attività 760 esistenti per l'anno selezionato
            var attivitaEsistenti = await _context.Attivita760
                .Include(a => a.Cliente)
            ...
            foreach (var cliente in clientiSenzaAttivita)
            {
                ...
                _context.Attivita760.Add(nuovaAttivita);
            }
            ...
            if (clientiSenzaAttivita.Any())
            {
                ...
                attivitaEsistenti = await _context.Attivita760
                    .Include(a => a.Cliente)
                    .Include(a => a.Professionista)
                    .Include(a => a.AnnoFiscale)
                    .Where(a => a.IdAnno == annoFiscale.IdAnno)
        ...
        // POST: Attivita760/BulkUpdate
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> BulkUpdate(List<Attivita760> attivita, int? annoSelezionato, IFormCollection form)
        {
            ...
            foreach (var item in attivita.Where(a => a.IdAttivita760 > 0))
            {
                try
                {
                    var esistente = await _context.Attivita760.FindAsync(item.IdAttivita760);
                    if (esistente != null)
                    {
                        ...
                        esistente.BilancioStato = item.BilancioStato;
                        esistente.BilancioInserimento = item.BilancioInserimento;
                        esistente.BilancioDeposito = item.BilancioDeposito;
        ...
        public async Task<IActionResult> DeleteSingle(int id)
        {
            try
            {
                ...
                var attivita = await _context.Attivita760
                    .Include(a => a.Cliente)
                    .Include(a => a.AnnoFiscale)
                    .FirstOrDefaultAsync(a => a.IdAttivita760 == id);
                ...
                // Elimina l'attività
                _context.Attivita760.Remove(attivita);
                await _context.SaveChangesAsync();

                // Log dell'operazione
                Console.WriteLine($"Eliminata attività Mod. 760 per cliente: {nomeCliente} (Anno: {anno})");

                return Json(new { 
                    success = true, 
                    message = $"Attività Mod. 760 per il cliente \"{nomeCliente}\" eliminata con successo."
                });
            }
            catch (Exception ex)
            {
        ...
        // POST: Attivita760/DeleteAllForYear
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteAllForYear(int anno)
        {
            try
            {
                ...
                var attivitaDaEliminare = await _context.Attivita760
                    .Where(a => a.IdAnno == annoFiscale.IdAnno)
                    .ToListAsync();
                ...
                _context.Attivita760.RemoveRange(attivitaDaEliminare);
                await _context.SaveChangesAsync();

                // Log dell'operazione (opzionale)
                Console.WriteLine($"Eliminati {count} inserimenti Mod. 760 per l'anno {anno}");
        ...
        public async Task<IActionResult> ExportExcel(int anno)
        {
            try
            {
                ...
                // Ottieni tutti i dati delle attività 760 per l'anno
                var attivita = await _context.Attivita760
                    .Include(a => a.Cliente)
                    .Include(a => a.Professionista)
                    .Include(a => a.AnnoFiscale)
                ...
                var worksheet = package.Workbook.Worksheets.Add($"Mod 760 - Anno {anno}");
                ...
                var headers = new[] { "Cliente", "ATECO", "Professionista", "Codice DR", "Raccolta Doc.",
                    "File ISA", "ISA DR Inseriti", "BilancioStato", "DR Inserita", "DR Controllata", "DR Spedita", 
                ...
                for (int i = 0; i < attivita.Count; i++)
                {
                    ...
                    worksheet.Cells[row, 8].Value = GetBilancioStatoText(item.BilancioStato);
                    ...
                    ApplySymbolFormatting(worksheet, row, 6, item.FileIsa); // File ISA
                ...
                worksheet.Tables.Add(dataRange, "Mod760Data");
                var table = worksheet.Tables["Mod760Data"];
                table.ShowHeader = true;
                table.ShowFilter = true;
                table.TableStyle = OfficeOpenXml.Table.TableStyles.Medium2;

                // Crea il file
                var fileName = $"Mod760_Anno_{anno}_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        private static string GetBilancioStatoText(string? bilancio)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateNote(int idAttivita, string note)
        {
            try
            {
                var attivita = await _context.Attivita760.FindAsync(idAttivita);
                if (attivita == null)
                {
                    return Json(new { success = false, message = "Attività non trovata." });
                }

                // Validazione lunghezza nota
                if (!string.IsNullOrEmpty(note) && note.Length > 500)
                {
                    return Json(new { success = false, message = "La nota non può superare i 500 caratteri." });
                }

                // Aggiorna la nota
                attivita.Note = string.IsNullOrWhiteSpace(note) ? null : note.Trim();
                attivita.UpdatedAt = DateTime.UtcNow;

                await _context.SaveChangesAsync();

                return Json(new { success = true, message = "Nota aggiornata con successo." });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Errore durante l'aggiornamento della nota per attività {idAttivita}: {ex.Message}");
